...
//		1. Stub endpoint
		stubFor(get(urlPathMatching("/api/.*")).willReturn(
				aResponse().withStatus(200).withHeader("Content-Type", "application/json").withBody(allCoinsBody)));
...
//		String resourceUrl = realServer;
		...
		ResponseEntity<String> response = restTemplate.getForEntity(resourceUrl + "/api/allcoins", String.class);
...
//		3. Verify
	...
	public void testBitcoin() {
		...
		stubFor(get(urlPathMatching("/api/.*")).willReturn(
				aResponse().withStatus(200).withHeader("Content-Type", "application/json").withBody(bitcoinBody)));

		RestTemplate restTemplate = new RestTemplate();
//		String resourceUrl = realServer;
		String resourceUrl = fakeServer;
		ResponseEntity<String> response = restTemplate.getForEntity(resourceUrl + "/api/bitcoin", String.class);
